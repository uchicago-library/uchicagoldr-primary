...
class PairTreeFileSystemStageReader(StageSerializationReader):
    ...
    def read(self):
        ...
        for x in [x.path for x in scandir(str(segments_dir))]:
            self.struct.add_segment(
                PairTreeFileSystemSegmentReader(
                    x,
...
class PairTreeFileSystemSegmentReader(SegmentPackager):
    ...
    def package(self):
        for ident in self._gather_identifiers():
            self.struct.add_materialsuite(
                PairTreeFileSystemMaterialSuiteReader(
                    self.path,
                    ident
                ).package()
            )
...
class PairTreeFileSystemMaterialSuiteReader(MaterialSuitePackager):
